...
AS $$
  SELECT CASE
    WHEN _diet IS NULL OR btrim(_diet) = '' THEN 'mixed'
    WHEN lower(replace(replace(btrim(_diet), '-', '_'), ' ', '_')) IN ('vegetarian', 'veg') THEN 'veg'
    ...
    WHEN lower(replace(replace(btrim(_diet), '-', '_'), ' ', '_')) = 'mixed' THEN 'mixed'
  ...
  FOR _d IN 0..29 LOOP
    ...
    FOREACH _slot IN ARRAY _slots LOOP
      ...
      ELSIF _slot = 'mid_bite' THEN
        ...
        ), CASE WHEN _is_vegan THEN 'Avocado' ELSE 'Greek Yogurt' END);
...
BEGIN
  ...
  IF _row.meal_slot = 'first_meal' THEN
    FOR _tries IN 1..4 LOOP
      SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation IN ('encourage','moderate') AND diet_type::text = ANY(_diet_filter) AND filter_id = CASE WHEN _use_nonveg AND NOT _is_vegan THEN _f_protein_nv ELSE _f_protein_veg END ORDER BY md5(id::text || _seed_txt || _tries::text || 'p') LIMIT 1;
    ...
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation IN ('encourage','moderate') AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_alt_grain AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'g') LIMIT 1;
    _items := _items || COALESCE(_candidate, 'Oats');
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation IN ('encourage','moderate') AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_fats AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'f') LIMIT 1;
    _items := _items || COALESCE(_candidate, 'Olive Oil');
  ELSIF _row.meal_slot = 'mid_bite' THEN
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation IN ('encourage','moderate') AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_nuts AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'n') LIMIT 1;
    _items := _items || COALESCE(_candidate, 'Almonds');
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation IN ('encourage','moderate') AND diet_type::text = ANY(_diet_filter) AND filter_id = CASE WHEN _is_vegan THEN _f_fats ELSE _f_dairy END AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'd') LIMIT 1;
  ...
  ELSE
    ...
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation IN ('encourage','moderate') AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_veg AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'v') LIMIT 1;
