...
CREATE OR REPLACE FUNCTION public.bbdo_user_diet_gating(_user_id uuid)
RETURNS TABLE(recs text[], allergen_ids uuid[], sub_prefs text[])
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
AS $$
DECLARE
  _is_intensive boolean;
  _sub text[];
  _allergens uuid[];
BEGIN
  SELECT EXISTS(
    SELECT 1 FROM public.subscriptions
    WHERE user_id = _user_id AND status = 'active' AND plan_id = 'intensive'
  ) INTO _is_intensive;

  SELECT COALESCE(sub_preferences, ARRAY[]::text[]),
         COALESCE(allergen_food_ids, ARRAY[]::uuid[])
  INTO _sub, _allergens
  FROM public.user_diet_profiles
  WHERE user_id = _user_id
  ORDER BY updated_at DESC NULLS LAST
  LIMIT 1;

  recs := CASE WHEN _is_intensive THEN ARRAY['encourage'] ELSE ARRAY['encourage','moderate'] END;
  allergen_ids := COALESCE(_allergens, ARRAY[]::uuid[]);
  sub_prefs := COALESCE(_sub, ARRAY[]::text[]);
  RETURN NEXT;
END;
$$;
  ...
  FOR _d IN 0..29 LOOP
    ...
    FOREACH _slot IN ARRAY _slots LOOP
      ...
      IF _slot = 'first_meal' THEN
        ...
        ), CASE WHEN _use_nonveg THEN 'Eggs' ELSE 'Tofu' END);
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation::text = ANY(_recs)
            AND diet_type::text = ANY(_diet_filter)
      ...
      ELSIF _slot = 'mid_bite' THEN
        ...
        ), CASE WHEN _skip_dairy THEN 'Avocado' ELSE 'Greek Yogurt' END);
      ...
      ELSE
        ...
        ), CASE WHEN _use_nonveg THEN 'Chicken' ELSE 'Paneer' END);
        _items := _items || COALESCE((
          SELECT name FROM public.food_items
          WHERE is_active AND recommendation::text = ANY(_recs)
            AND diet_type::text = ANY(_diet_filter)
