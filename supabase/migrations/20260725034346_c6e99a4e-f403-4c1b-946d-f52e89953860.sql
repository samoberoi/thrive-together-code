...
BEGIN
  ...
  ELSIF _is_nonveg OR _resolved_diet = 'mixed' THEN _diet_filter := ARRAY['vegan','veg','non_veg'];
  ...
  IF _row.meal_slot = 'first_meal' THEN
    ...
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_alt_grain AND NOT (id = ANY(_allergens)) AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'g') LIMIT 1;
    _items := _items || COALESCE(_candidate, 'Oats');
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_fats AND NOT (id = ANY(_allergens)) AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'f') LIMIT 1;
    _items := _items || COALESCE(_candidate, 'Olive Oil');
  ELSIF _row.meal_slot = 'mid_bite' THEN
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_nuts AND NOT (id = ANY(_allergens)) AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'n') LIMIT 1;
    _items := _items || COALESCE(_candidate, 'Almonds');
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = CASE WHEN _skip_dairy THEN _f_fats ELSE _f_dairy END AND NOT (id = ANY(_allergens)) AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'd') LIMIT 1;
  ...
  ELSE
    ...
    SELECT name INTO _candidate FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_veg AND NOT (id = ANY(_allergens)) AND NOT (name = ANY(_old_items)) ORDER BY md5(id::text || _seed_txt || 'v') LIMIT 1;
...
-- Fix generate_diet_plating similarly so newly generated plates use real data
...
BEGIN
  ...
  FOR _d IN 0..29 LOOP
    _use_nonveg := CASE
      WHEN _is_vegan OR (_is_veg AND NOT _is_nonveg AND NOT _is_eggitarian) THEN false
      WHEN _is_nonveg THEN ((_d + floor(random() * 3)::integer) % 3 <> 1)
    ...
    FOREACH _slot IN ARRAY _slots LOOP
      ...
      IF _slot = 'first_meal' THEN
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = CASE WHEN _use_nonveg AND NOT _is_vegan THEN _f_protein_nv ELSE _f_protein_veg END AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'fp') LIMIT 1), CASE WHEN _use_nonveg THEN 'Eggs' ELSE 'Tofu' END);
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_alt_grain AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'fg') LIMIT 1), 'Oats');
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_fats AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'ff') LIMIT 1), 'Olive Oil');
      ELSIF _slot = 'mid_bite' THEN
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_nuts AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'mn') LIMIT 1), 'Almonds');
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = CASE WHEN _skip_dairy THEN _f_fats ELSE _f_dairy END AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'md') LIMIT 1), CASE WHEN _skip_dairy THEN 'Avocado' ELSE 'Greek Yogurt' END);
      ELSE
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = CASE WHEN _use_nonveg AND NOT _is_vegan THEN _f_protein_nv ELSE _f_protein_veg END AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'lp') LIMIT 1), CASE WHEN _use_nonveg THEN 'Chicken' ELSE 'Paneer' END);
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_veg AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'lv') LIMIT 1), 'Broccoli');
        _items := _items || COALESCE((SELECT name FROM public.food_items WHERE is_active AND recommendation::text = ANY(_recs) AND diet_type::text = ANY(_diet_filter) AND filter_id = _f_fats AND NOT (id = ANY(_allergens)) ORDER BY md5(id::text || _d::text || _seed || 'lf') LIMIT 1), 'Avocado');
      END IF;
