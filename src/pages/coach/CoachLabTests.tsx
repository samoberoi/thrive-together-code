            ...
            {filteredTests.map((t) => {
              ...
              return (
                <motion.div key={t.id} onClick={() => toggleTest(t.id)} whileTap={{ scale: 0.98 }} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleTest(t.id); } }} className={`w-full text-left liquid-glass rounded-2xl p-4 flex items-start gap-3 transition cursor-pointer ${checked ? "ring-2 ring-primary" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0"><div className="font-semibold text-sm">{t.product_name}</div><div className="text-xs text-muted-foreground mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-1"><span>Code: {t.product_code}</span>{t.category && <span>{t.category}</span>}<button type="button" onClick={(e) => { e.stopPropagation(); setParamsTest(t); }} className="inline-flex items-center gap-1 rounded-full border border-primary/30 bg-primary/10 px-2 py-0.5 font-semibold text-primary hover:bg-primary/20 transition"><ListChecks className="w-3 h-3" />View details</button>{t.fasting_required && <Badge variant="secondary" className="text-[10px]">Fasting</Badge>}</div></div>
